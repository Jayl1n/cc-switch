...
pub struct ProxyService {
    ...
    /// CoreEvents callback, passed to ProxyServer for failover notifications
    events: Arc<RwLock<Option<Arc<dyn CoreEvents>>>>,
...
impl ProxyService {
    ...
    /// Set CoreEvents callback (call during app initialization)
    pub fn set_events(&self, handle: Arc<dyn CoreEvents>) {
        futures::executor::block_on(async {
            *self.events.write().await = Some(handle);
        });
    }
    ...
    pub async fn start(&self) -> Result<ProxyServerInfo, String> {
        ...
        let server = ProxyServer::new(config.clone(), self.db.clone(), events);
        let info = server
            .start()
            .await
                ...
                if let Ok(Some(provider)) = self.db.get_provider_by_id(&current_id, app_type_str) {
                    if provider.category.as_deref() == Some("official") {
                        if let Some(events) = self.events.read().await.as_ref() {
                            events.emit_proxy_flags_changed(&crate::events::ProxyFlags {
                                running: true,
                                claude: false,
                                codex: false,
                                gemini: false,
    ...
    pub async fn update_config(&self, config: &ProxyConfig) -> Result<(), String> {
        ...
        if require_restart {
            ...
            let events = self.events.read().await.clone();
            let new_server = ProxyServer::new(new_config.clone(), self.db.clone(), events);
            let info = new_server
                .start()
                .await
