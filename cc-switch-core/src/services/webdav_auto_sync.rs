...
use serde_json::json;
use crate::events::{CoreEvents, SyncStatus};
...
fn emit_auto_sync_status_updated(events: &Arc<dyn CoreEvents>, status: &str, error: Option<&str>) {
    ...
    events.emit_sync_status(&SyncStatus { backend: "webdav".into(), status: status.to_string(), message: error.map(|e| e.to_string()), timestamp: chrono::Utc::now().timestamp() });
}
...
async fn run_auto_sync_upload(
    ...
    match result {
        Ok(_) => {
            emit_auto_sync_status_updated(events, "success", None);
            Ok(())
        }
        Err(err) => {
            persist_auto_sync_error(&mut sync_settings, &err);
            emit_auto_sync_status_updated(events, "error", Some(&err.to_string()));
            Err(err)
        }
    }
}
...
pub fn start_worker(db: Arc<crate::database::Database>, events: Arc<dyn CoreEvents>) {
    ...
    tokio::spawn(async move {
        run_worker_loop(db, rx, events).await;
    });
}
...
async fn run_worker_loop(
    ...
    while let Some(first_table) = rx.recv().await {
        ...
        if let Err(err) = run_auto_sync_upload(&db, &events).await {
