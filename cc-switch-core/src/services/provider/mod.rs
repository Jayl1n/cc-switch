...
"#;
    ...
    async fn update_current_claude_provider_syncs_live_when_proxy_takeover_detected_without_backup()
    {
        ...
        db.update_proxy_config(ProxyConfig {
            ...
            listen_port: 0, // ephemeral port to avoid conflicts in CI / parallel tests
            ..Default::default()
        })
        .await
        ...
        // Get the actual proxy address (ephemeral port)
        let proxy_base_url = {
            let config = db
                .get_proxy_config()
                .await
            ...
            let port = if config.listen_port != 0 {
                ...
                // Port was ephemeral; read it back from the DB (persist_ephemeral_listen_port)
                db.get_proxy_config()
                    .await
                    .expect("get proxy config again")
                    .listen_port
            };
    ...
    async fn update_current_claude_desktop_provider_syncs_profile_when_proxy_takeover_is_active() {
        ...
        // Use ephemeral port to avoid conflicts in CI / parallel tests
        {
            let mut pc = db
                .get_proxy_config()
                .await
