...
use std::sync::Arc;
// tauri::Manager removed — using AuthProvider trait instead
...
pub struct RequestForwarder {
    ...
    /// 事件回调（替代 Tauri AppHandle），用于发射事件
    events: Option<Arc<dyn CoreEvents>>,
    /// 认证提供者（Copilot/Codex OAuth），用于获取动态 token
...
impl RequestForwarder {
                ...
                Ok((response, claude_api_format, outbound_model)) => {
                    ...
                    // 更新成功统计
                    {
                        ...
                        if should_switch {
                            ...
                            tokio::spawn(async move {
                                let _ = fm.try_switch(ev.as_deref(), &at, &pid, &pname).await;
                            });
                        }
                        // 重新计算成功率
                        if status.total_requests > 0 {
                                ...
                                Ok((response, claude_api_format, outbound_model)) => {
...

                                    {
                                        ...
                                        if should_switch {
                                            ...
                                            tokio::spawn(async move {
                                                let _ = fm
                                                    .try_switch(ev.as_deref(), &at, &pid, &pname)
                                                    .await;
                                            });
                                        }
                                        if status.total_requests > 0 {
    ...
    async fn forward(
        ...
        // 从 AuthProvider 获取缓存的 API endpoint（支持企业版等非默认 endpoint）
        if is_copilot && !is_full_url {
            if let Some(auth) = &self.auth_provider {
                ...
                if let Some(dynamic_endpoint) = auth.get_copilot_endpoint(account_id.as_deref()) {
        ...
        let mut auth_headers = if let Some(mut auth) = adapter.extract_auth(provider) {
            // GitHub Copilot 特殊处理：通过 AuthProvider 获取真实 token
            if auth.strategy == AuthStrategy::GitHubCopilot {
                if let Some(ap) = &self.auth_provider {
                    let account_id = provider
                        .meta
                        .as_ref()
                        .and_then(|m| m.managed_account_id_for("github_copilot"));

                    match ap.get_copilot_token(account_id.as_deref()) {
                        Ok(token) => {
                    ...
                    log::error!("[Copilot] AuthProvider 不可用");
                    return Err(ProxyError::AuthError(
                        "GitHub Copilot 认证不可用（无 AuthProvider）".to_string(),
                    ));
                }
            }

            // Codex OAuth 特殊处理：通过 AuthProvider 获取真实 access_token
            if auth.strategy == AuthStrategy::CodexOAuth {
                if let Some(ap) = &self.auth_provider {
                    ...
                    match ap.get_codex_token(account_id.as_deref()) {
                        Ok(token) => {
                            auth = AuthInfo::new(token, AuthStrategy::CodexOAuth);
                            should_send_codex_oauth_session_headers = true;
                            codex_oauth_account_id = ap.get_codex_account_id(account_id.as_deref());
                            log::debug!(
                    ...
                    log::error!("[CodexOAuth] AuthProvider 不可用");
                    return Err(ProxyError::AuthError(
                        "Codex OAuth 认证不可用（无 AuthProvider）".to_string(),
                    ));
                }
            }
    ...
    async fn apply_copilot_live_model_resolution(
        ...
        let Some(_auth) = &self.auth_provider else {
            return;
        };
        let account_id = provider
            .meta
        ...
        // Note: live model resolution via AuthProvider requires the implementor
        // to expose models. For now, skip in embedded mode (no-op).
        // Desktop mode implements this via TauriEvents wrapper.
        let _ = (account_id, model_id);
    }

    async fn is_copilot_openai_vendor_model(&self, provider: &Provider, model_id: &str) -> bool {
        let Some(ap) = &self.auth_provider else {
            log::debug!("[Copilot] AuthProvider unavailable, fallback to chat/completions");
            return false;
        };
        ...
        ap.is_copilot_openai_vendor_model(account_id.as_deref(), model_id)
    }
