...
impl FailoverSwitchManager {
    ...
    /// 注入 AppState（桌面版在 setup 时调用，嵌入式模式可选）
    pub async fn set_app_state(&self, state: Arc<AppState>) {
        *self.app_state.write().await = Some(state);
    }
    ...
    pub async fn try_switch(
        ...
        let result = self
            .do_switch(events, app_type, provider_id, provider_name)
            .await;
    ...
    async fn do_switch(
        ...
        // 尝试通过 AppState 执行热切换
        if let Some(app_state) = self.app_state.read().await.as_ref() {
            switched = app_state
                .proxy_service
        ...
        if let Some(events) = events {
            events.emit_provider_switched(app_type, provider_id, provider_name);
        }
