...
/// Runtime override for the app config directory.
/// In desktop mode this is set by `app_store.rs` (Tauri Store);
/// in embedded mode it can be set via `CC_SWITCH_CONFIG_DIR` env var
/// or programmatically via `set_app_config_dir_override()`.
static APP_CONFIG_DIR_OVERRIDE: RwLock<Option<PathBuf>> = RwLock::new(None);

/// Programmatically set the app config directory override.
///
/// Can be called multiple times (last value wins).
/// Must be called before any `get_app_config_dir()` call.
/// If not called, falls back to `CC_SWITCH_CONFIG_DIR` env var,
/// then `~/.cc-switch`.
pub fn set_app_config_dir_override(path: Option<PathBuf>) {
    if let Ok(mut guard) = APP_CONFIG_DIR_OVERRIDE.write() {
        *guard = path;
    }
}

/// Get the current app config directory override (if set).
pub fn get_app_config_dir_override() -> Option<PathBuf> {
    if let Ok(guard) = APP_CONFIG_DIR_OVERRIDE.read() {
